...
class CRNNRecognizer():
    ...
    def generate_test_data(self, word_list, samples_per_word=1):
        ...
        for w in word_list:
            ...
            all_imgs = np.transpose(all_imgs,
                                    (1, 0, 2, 3))  # (samples_per_word, self.word_length, self.height, self.width)
...
if __name__ == "__main__":
    ...
    if sys.argv[1] == 'test':
        ...
        X, Y = d.get_test_dataset()
        with open('input.txt', 'w', encoding='utf8') as f:
            f.write('\n'.join(m.labels_to_words(Y)))
